...
use crate::html::parser::{parse_fragment, parse_html};
use crate::html::select::*;
...
"#;
    ...
    assert_eq!(&**child.as_text().unwrap().borrow(), "Foo");
    assert_eq!(
        matching[0].attributes.borrow().get("class"),
        Some("foo bar")
    );
    assert_eq!(
        matching[0].attributes.borrow().get(local_name!("class")),
    ...
    assert_eq!(
        document.select("p.bar").unwrap().collect::<Vec<_>>().len(),
        1,
    );
    assert_eq!(
        document.select("p.baz").unwrap().collect::<Vec<_>>().len(),
        1,
    );
    assert_eq!(
        document
